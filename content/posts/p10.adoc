...
A friend told me recently he was solving a problem about polygons.
...
So for an N-gon the number of diagonals is +++$\binom{N}{2} - N = \frac{N(N-3)}{2}$+++ (for proofs see
...
Sidenote: One of those threads has the title "Number of Diagonals in
...
If I have some time I'll probably think more about ways
to derive that formula. The problem in its original form was solved.
I had some fun writing this but I'll set the problem aside for now.
